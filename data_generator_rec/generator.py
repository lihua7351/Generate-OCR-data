...
# dict_3 = to_dictionary('generator.txt', 'utf-8')
...
def random_word_color():
    ...
    # font_color_choice = [[200,245,145],[235,244,165],[220,244,160],[244,244,200],[150,180,110]]
    # font_color_choice = [[222,245,155],[152,170,100],[183,205,114],[200,237,139],[117,151,93],[132,156,89]]
...
def main(save_path, num, file):
    ...
    file.write('train/' + str(num)+ '.png ' + random_word + '\n')
    raw_image.save(save_path+str(num)+'.png')

if __name__ == '__main__':
   
    # 处理具有工商信息语义信息的语料库，去除空格等不必要符号
    with open('generator1.txt', 'r', encoding='utf-8') as file:
        info_list = [part.strip().replace('\t', '') for part in file.readlines()]
        info_str = ''.join(info_list)
        # print(info_str)

    # 图片标签
    file  = open('data_set/train1.txt', 'w', encoding='utf-8')
    total = 100000
    for num in range(0,total):
        main('data_set/train/', num+100000, file)
        if num % 200000 == 0:
            print('[%d/%d]'%(num,total))
    file.close()
